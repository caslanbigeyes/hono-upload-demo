...
export class ChinesePDFGenerator {
  ...
  private setupChineseFont() {
    try {
      // 尝试使用系统中文字体 - 只使用TTF格式的字体
      const fontPaths = [
        '/System/Library/Fonts/PingFang.ttf', // macOS (如果存在TTF版本)
        '/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf', // Linux
        'C:\\Windows\\Fonts\\msyh.ttf', // Windows 微软雅黑 TTF
        'C:\\Windows\\Fonts\\simsun.ttf', // Windows 宋体 TTF
      ]
      ...
      if (!this.chineseFontRegistered) {
        console.log('No Chinese font found, using Helvetica as fallback')
        // 对于中文字符，我们将使用特殊处理
      }
    } catch (error) {
