...
personalInfo.openapi(getPersonalInfoRoute, jwtMiddleware, async (c) => {
  try {
...
personalInfo.openapi(upsertPersonalInfoRoute, jwtMiddleware, async (c) => {
  try {
    const currentUser = AuthService.getCurrentUser(c)
    if (!currentUser) {
      return c.json({ error: 'User not found' }, 401)
...
personalInfo.openapi(deletePersonalInfoRoute, jwtMiddleware, async (c) => {
  try {
    const currentUser = AuthService.getCurrentUser(c)
    if (!currentUser) {
      return c.json({ error: 'User not found' }, 401)
